...
class Writer:
	...
	def replace(self,message):
		...
		message = re.sub(r"\$\{([_a-z]+)(?:\,([_a-z]+))?\s?(b|bold)?\s?(u|ul|under|line|underline)?\}",self.set_colours,message)
		message = re.sub(r"\$\{[a-z]+\}","",message)
		return message

	def set_colours(self,match):
		...
		if undr:
			...
			bits.append("24")
		if fore in Writer.foreColours.keys():
			bits.append(Writer.foreColours[fore])
		else:
			bits.append("39")
		if back in Writer.backColours.keys():
			bits.append(Writer.backColours[back])
		else:
			bits.append("49")
	...
	def write(self,message):
		print(self.replace(message))

if __name__ == "__main__":
	w = Writer()
	w.write("hello ${yellow}this${reset} is a test")
